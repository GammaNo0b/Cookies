...
public class LumberAxe extends AbstractCustomItem {
	...
	public void onBlockBreak(Player player, ItemStack stack, BlockBreakEvent event) {
		if(event.getBlock().getType().toString().contains("_WOOD") || event.getBlock().getType().toString().contains("_LOG")) {
			...
			int brokenBlocks = breakWood(nextGeneration, woodBlocks, 0);
			ItemMeta meta = stack.getItemMeta();
			if(meta instanceof Damageable) {
				Damageable damageable = (Damageable) meta;
				damageable.setDamage(Math.max(stack.getType().getMaxDurability(), damageable.getDamage() + (int) (((double) brokenBlocks) / (meta.getEnchantLevel(Enchantment.DURABILITY) + 1.0D))));
	...
	private int breakWood(Map<Location, Block> currentGeneration, Map<ItemStack, Integer> woodBlocks, int amount) {
		if(!currentGeneration.isEmpty()) {
			...
			amount = breakWood(nextGeneration, woodBlocks, amount);
		}
		return amount;
	}
