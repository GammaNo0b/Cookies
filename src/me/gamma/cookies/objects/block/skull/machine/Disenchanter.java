...
public class Disenchanter extends AbstractGuiProvidingSkullBlock implements BlockTicker, Switchable, ItemConsumer, ItemSupplier {
	...
	public static final ItemStackProperty OUTPUT = new ItemStackProperty("outupt");
	...
	private void fillInventoryMap() {
		for(Location location : this.getLocations()) {
			if(location.getBlock().getState() instanceof TileState) {
				...
				gui.setItem(21, new ItemStack(Material.BOOK, BOOKS.fetch(block)));
				gui.setItem(23, OUTPUT.fetch(block));
				gui.setItem(24, ENCHANTED_BOOK.fetch(block));
	...
	private Inventory createInventory(TileState block) {
		...
		filler = InventoryManager.filler(Material.YELLOW_STAINED_GLASS_PANE);
		for(int i : new int[] {
			2, 3, 5, 6
		})
			gui.setItem(i, filler);
	...
	private void setBooks(TileState block, int books) {
		this.inventories.get(block.getLocation()).setItem(21, new ItemStack(Material.BOOK, books));
	...
	private void setEnchantedBook(TileState block, ItemStack book) {
		this.inventories.get(block.getLocation()).setItem(24, book);
	...
	public ItemStack onBlockBreak(Player player, TileState block, BlockBreakEvent event) {
		...
		Utilities.dropItem(processing, block.getLocation());
		if(!Utilities.isEmpty(processing))
			Utilities.dropItem(new ItemStack(Material.BOOK), block.getLocation());
	...
	public boolean onMainInventoryInteract(Player player, TileState block, Inventory gui, InventoryClickEvent event) {
		...
		if(slot == 20)
			return false;
		if(slot == 21)
			return !Utilities.isEmpty(cursor) && cursor.getType() != Material.BOOK;
		if(slot == 23 || slot == 24)
	...
	public void tick(TileState block) {
		...
		ItemStack bookstack = inventory.getItem(21);
		int books = Utilities.isEmpty(bookstack) ? 0 : bookstack.getAmount();
		BOOKS.store(block, books);
		ENCHANTED_BOOK.store(block, inventory.getItem(24));
		...
		if(progress < 100 && !Utilities.isEmpty(processing))
			progress++;
		if(progress >= 100 && !Utilities.isEmpty(processing)) {
			if(Utilities.isEmpty(OUTPUT.fetch(block)) && Utilities.isEmpty(ENCHANTED_BOOK.fetch(block))) {
				ItemStack book = new ItemStack(Material.ENCHANTED_BOOK);
				EnchantmentStorageMeta meta = (EnchantmentStorageMeta) book.getItemMeta();
				for(Enchantment enchantment : processing.getEnchantments().keySet())
					meta.addStoredEnchant(enchantment, processing.removeEnchantment(enchantment), true);
				book.setItemMeta(meta);
				this.setOutput(block, processing);
				PROCESSING.storeEmpty(block);
				processing = null;
				this.setEnchantedBook(block, book);
				progress = 0;
			}
		}
		if(Utilities.isEmpty(processing)) {
			ItemStack input = INPUT.fetch(block);
			if(books > 0 && !Utilities.isEmpty(input) && input.getEnchantments().size() > 0) {
	...
	public String getRegistryName() {
		return "disentchanter";
	}


	@Override
	public String getDisplayName() {
		return "§6Disenchanter";
	}
