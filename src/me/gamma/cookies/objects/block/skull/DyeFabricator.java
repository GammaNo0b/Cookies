...
public class DyeFabricator extends AbstractGuiProvidingSkullBlock {
	...
	public Inventory createMainGui(Player player, TileState block) {
		...
		gui.setItem(9, new ItemBuilder(Material.RED_STAINED_GLASS_PANE).setName("§4Red: §c" + RED.fetch(block)).build());
		...
		gui.setItem(18, new ItemBuilder(Material.GREEN_STAINED_GLASS_PANE).setName("§2Green: §a" + GREEN.fetch(block)).build());
		...
		gui.setItem(27, new ItemBuilder(Material.BLUE_STAINED_GLASS_PANE).setName("§1Blue: §9" + BLUE.fetch(block)).build());
			...
			if(cursor != null && cursor.getItemMeta() instanceof LeatherArmorMeta) {
				...
				if(!color.equals(meta.getColor())) {
					if(this.removeColor(block, color)) {
						...
						this.handleCursor(player, cursor);
						player.openInventory(this.createMainGui(player, block));
					}
				}
			}
	...
	private void handleCursor(Player viewer, ItemStack cursor) {
		if(cursor != null) {
			Map<Integer, ItemStack> rest = viewer.getInventory().addItem(cursor.clone());
			cursor.setAmount(0);
			if(!rest.isEmpty()) {
