...
public interface MachineInventorySupplier {
	...
	public static final MachineInventorySupplier BASIC_INVENTORY_SUPPLIER = (location, machine) -> {
		...
		for(int i : machine.getOutputSlots())
		...
		gui.setItem(machine.getProgressSlot(), machine.getProgressIcon());
		gui.setItem(machine.getIdentifierSlot(), new ItemBuilder(Material.GRAY_STAINED_GLASS_PANE).setName("§9Location: §3" + location.getBlockX() + " : " + location.getBlockY() + " : " + location.getBlockZ()).build());
		return gui;

	};
	public static final MachineInventorySupplier ADVANCED_INVENTORY_SUPPLIER = (location, machine) -> {
		Inventory gui = Bukkit.createInventory(null, machine.getRows() * 9, machine.getDisplayName());
		for(int i = 0; i < 9; i++) {
			gui.setItem(i, new ItemBuilder(Material.GRAY_STAINED_GLASS_PANE).setName(" ").build());
			gui.setItem(gui.getSize() - 1 - i, new ItemBuilder(Material.GRAY_STAINED_GLASS_PANE).setName(" ").build());
	...
	public static final MachineInventorySupplier IMPROVED_INVENTORY_SUPPLIER = (location, machine) -> {
		Inventory gui = Bukkit.createInventory(null, machine.getRows() * 9, machine.getDisplayName());
	...
	public static final MachineInventorySupplier PERFECTED_INVENTORY_SUPPLIER = (location, machine) -> {
		Inventory gui = Bukkit.createInventory(null, machine.getRows() * 9, machine.getDisplayName());
		gui.setItem(40, new ItemBuilder(Material.GRAY_STAINED_GLASS_PANE).setName(" ").build());
		for(int i : machine.getOutputSlots())
			gui.setItem(i, new ItemBuilder(Material.BARRIER).setName(" ").build());
