...
public abstract class AbstractGuiMachine extends AbstractMachine implements UpdatingGuiProvider, BlockFaceConfigurable {

	protected final Map<Location, Inventory> inventories = new HashMap<>();

	public AbstractGuiMachine(MachineTier tier) {
		super(tier);
	}
	...
	public boolean load(TileState block, PersistentDataObject data) {
		return super.load(block, data) && UpdatingGuiProvider.super.load(block, data);
	}


	@Override
	public boolean save(TileState block, PersistentDataObject data) {
		return super.save(block, data) && UpdatingGuiProvider.super.save(block, data);
	}
	...
	public Inventory createGui(TileState block) {
		return MachineConstants.createGui(this, block);
	}
	...
	public boolean onBlockRightClick(Player player, TileState block, ItemStack stack, PlayerInteractEvent event) {
		...
		if(!this.canAccess(block, player)) {
			player.sendMessage("§cYou don't own this machine!");
			return true;
		}
	...
	public boolean onMainInventoryInteract(Player player, TileState block, Inventory gui, InventoryClickEvent event) {
		...
		if(slot == this.getUpgradeSlot() && this.getUpgradeSlots(block) > 0) {
