...
public class StorageExporter extends AbstractCustomBlock implements NetworkInterface<ItemStack>, AdvancedFilterBlock<ItemStack, ItemFilter>, Cartesian {
	...
	public List<Provider<ItemStack>> getOutputs(TileState block) {
		List<Provider<ItemStack>> outputs = new ArrayList<>();
		...
		for(BlockFace face : BlockUtils.cartesian) {
			...
			if(state instanceof TileState tile) {
				if(Blocks.getCustomBlockFromBlock(tile) instanceof ItemConsumer consumer) {
					outputs.addAll(consumer.getItemInputs(tile, face.getOppositeFace()));
					continue;
				}
			}

			if(state instanceof BlockInventoryHolder holder)
				outputs.addAll(ItemProvider.fromInventory(holder.getInventory()));
		}
		return outputs;
	}
	...
	public boolean onBlockRightClick(Player player, TileState block, ItemStack stack, PlayerInteractEvent event) {
		if(!player.isSneaking()) {
			ItemFilterGui.open(player, block, this, BLOCK_FACE_ACCESS_FLAGS, Material.RED_STAINED_GLASS_PANE);
			return true;
		}
