...
import me.gamma.cookies.object.gui.util.ColorGrid.Data;
...
public class ColorGrid implements InventoryProvider<Data> {
	...
	private void update(Inventory gui, Data data) {
		...
		for(int i = 0; i < 4; i++) {
			for(int j = 0; j < 4; j++) {
				int c = palette[i][j];
				gui.setItem(i * 9 + j + 10, data.createIcon(c, c == data.color.get().asRGB()));
			}
		}
		...
		gui.setItem(16, data.createIcon(data.color.get().asRGB(), false));
	}
	...
	public boolean onMainInventoryInteract(Player player, Data data, Inventory gui, InventoryClickEvent event) {
		...
		if(slot == 4) {
			...
			int c = slot % 9;
			if(3 <= r && r < 5 && 6 <= c && c < 9) {
				data = new Data(data.color, Sector.grid[r - 3][c - 6]);
				this.openGui(player, data, false, false);
			} else if(1 <= r && r < 5 && 1 <= c && c < 5) {
				data.color.set(Color.fromRGB(data.getPalette()[r - 1][c - 1]));
				this.update(gui, data);
			}
		}
	...
	public static void openColorWheel(HumanEntity player, Holder<Color> color) {
		Inventories.COLOR_WHEEL.openGui(player, new Data(color, Sector.RED_MAJOR));
	}

	record Data(Holder<Color> color, Sector sector) {

		private void storeColor() {
			this.color.set(Color.fromRGB(this.color.get().asRGB()));
		}
